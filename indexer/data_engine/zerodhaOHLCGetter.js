...
const writeWeeklyCandles = (
  ...
  while (moment(startTime).isBefore(endTime)) {
    ...
    yearCanedles = candles.filter((candle) => {
      ...
      return (
        moment(candle[0]).isBefore(nextYearDate) &&
        moment(candle[0]).isAfter(startTime)
      );
    });
    ...
    zerodhaUtil
      ...
      .then((candles) => {
        try {
          ...
          console.log(err);
          console.error(
            `Faield to fetch ${tickName}, start: ${startTime}, duration:${timeFrame}`
          );
        } finally {
          setTimeout(() => {
            resolve();
...
// getQuotesOfPeriod("15minute", "2020-01-01", "2020-12-31");
// getQuotesOfPeriod("15minute", "2017-01-01", "2017-12-31");
// getQuotesOfPeriod("15minute", "2018-01-01", "2018-12-31");
// getQuotesOfPeriod("15minute", "2019-01-01", "2019-12-31");
// getQuotesOfPeriod("15minute", "2020-01-01", "2020-12-31");
