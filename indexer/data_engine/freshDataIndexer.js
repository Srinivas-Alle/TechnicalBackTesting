...
const directoryPath =
  "/Users/srinivasalle/Desktop/workspace/za/TechnicalBackTesting/indexer/data_engine/zerodha_data";

const niftyQuotes = JSON.parse(
  fs.readFileSync(
    "/Users/srinivasalle/Desktop/workspace/za/TechnicalBackTesting/indexer/data_engine/NSE_FUTURES_listed_EQ.json",
    "utf8"
  )
);
...
const writeCandlesToFile = (candles, year, name, token, timeFrame) =>
  new Promise((resolve) => {
    const filePath = `${directoryPath}/${timeFrame}/${year}/${name}_${token}.json`;
    fsExtra.ensureFileSync(filePath);
    let existingTicks = [];
    try {
      existingTicks = JSON.parse(fs.readFileSync(filePath, "utf-8"));
    } catch (e) {}
...
const requestOHLCOf = async (
  ...
  const candles = await zerodhaUtil.requestBySplittingTime(
    name,
    instrumentToken,
    timeFrame,
    startTime,
...
async function applyTechnicals(
  candles,
  name,
  instrumentToken,
  startTime,
  ...
  let last200Canelds = await elasticUtil.search(
    getLast200Candles(name, startTime),
    `ticks_${timeFrame}`
  );
...
async function indexAllTicksOfPeriod(timeFrame, tillDate) {
  for (let index = 0; index < niftyQuotes.length; index += 1) {
    ...
    const reslutls = await elasticUtil.search(
      getQueryOfLastEntry(name),
      `ticks_${timeFrame}`
    );
    ...
    if (moment(startTime).isAfter(moment(tillDate)))
      throw new Error("invalid start & end times");

    const ohlcCandles = await requestOHLCOf(
      name,
      instrument_token,
      timeFrame,
      startTime,
    ...
    const emaTicks = await applyTechnicals(
      ohlcCandles,
      name,
      instrument_token,
      startTime,
