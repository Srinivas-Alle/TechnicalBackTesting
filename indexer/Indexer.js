...
const allQuotesWithTicks = niftyQuotes.map(
  (quote) => `${quote.name}_${quote.instrument_token}`
);
...
async function indexTicksInSmallerTime(timeFrame) {
  for (
    let quoteIndex = 0;
    quoteIndex < allQuotesWithTicks.length;
    quoteIndex += 1
  ) {
    ...
    for (let yearIndex = 0; yearIndex < years.length; yearIndex += 1) {
      ...
      console.log(
        "Log output: indexTicksOf -> stockTicks",
        stockTicks.length,
        quote,
        years[yearIndex],
        timeFrame
